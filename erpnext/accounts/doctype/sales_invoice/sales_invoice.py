...
@frappe.whitelist()
def set_price_list():
	price_list = frappe.db.sql("""select selling_price_list from `tabPOS Profile` where user = '%s' """%(frappe.session.user),as_list=1)
	if price_list:
		return price_list
	else:
		price_list = frappe.db.sql("""select name from `tabPrice List` where selling=1 and enabled=1 """,as_list=1)
		if not price_list:
...
# 		return price_list
# 	else:
# 		price_list = frappe.db.sql("""select name from `tabPrice List` where selling=1 and enabled=1 and company='%s'"""%(company),as_list=1)
# 		if not price_list:
# 			frappe.msgprint(_("Please create Price List first."))
# 		else:
# 			return price_list
